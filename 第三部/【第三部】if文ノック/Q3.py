...
amount = float(input('お買上金額を入力してください。:'))  # お買上金額
tax_rate = 0.1  # 消費税率
is_food = int(input('以下の数値を入力してください。\n0) 食料品以外\n1) 食料品\n:'))
is_takeout = int(input('以下の数値を入力してください。\n0) 店内飲食\n1) 持ち帰り\n:'))

# tax_amount = amount * tax_rate # お買い上げ金額×消費税率

if is_food == 0:
    if is_takeout == 0 or is_takeout == 1:
        tax_amount = amount *(1 + tax_rate)
        float(print(tax_amount))
    else:
        print('不正な値です')
elif is_food == 1:
    if is_takeout == 0:
